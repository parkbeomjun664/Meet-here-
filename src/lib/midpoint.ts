/**
 * midpoint.ts — 중간지점 탐색 알고리즘 (이 앱의 두뇌)
 *
 * UI가 전혀 없는 순수 로직 파일. React와 무관해서 테스트하기 쉽다.
 * 무거운 모듈이라 버튼 클릭 시점에 동적 import(코드 스플리팅)된다.
 *
 * 핵심 아이디어:
 *   좌표의 중심을 그냥 쓰지 않는다. 후보 지점들을 실제로 평가해서
 *   "공평성 + 평균 소요시간 + 교통비"가 가장 좋은 곳을 고른다.
 *
 * 흐름:
 *   1. 멤버 좌표 평균 → 중심점
 *   2. 멤버 간 최대 거리 → 검색 전략 결정(지하철역/터미널/KTX역)
 *   3. 중심점 주변 후보 검색 (카카오 로컬 API, 최대 8곳)
 *   4. 후보 × 멤버 → 경로·시간·비용 계산 (멤버는 병렬, 후보는 순차)
 *   5. 세 지표를 0~1로 정규화 후 가중 합산 → 점수 최저 후보 선택
 *
 * 자세한 설계·수식·한계는 docs/algorithm.md 참고.
 *
 * 주요 export:
 *   - findMidpoint()        중간지점 탐색 (메인)
 *   - recommendTransport()  이동수단 추천 (약속잡기 탭의 "추천" 버튼)
 *   - calcDistance()        Haversine 직선거리
 */
import type {
  MidpointResult,
  UserEntry,
